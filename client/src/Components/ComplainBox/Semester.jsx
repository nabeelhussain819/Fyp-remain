...
const SemesterComplain = ({ data }) => {
  const [complain, setComplain] = useState("");
  const [studentId, setUserId] = useState(localStorage.getItem("id"));
  const [semesterId, setSemesterId] = useState([data._id]);

  const ComplainDept = async (e) => {
    e.preventDefault();
    const res = await fetch("http://localhost:5000/semester-complain", {
      method: "POST",
...
export default SemesterComplain;
