...
const ProgramComplain = ({ data }) => {
  const [complain, setComplain] = useState("");
  const [studentId, setUserId] = useState(localStorage.getItem("id"));
  const [programId, setProgramId] = useState([data._id]);

  const ComplainDept = async (e) => {
    e.preventDefault();
    const res = await fetch("http://localhost:5000/program-complain", {
      method: "POST",
...
export default ProgramComplain;
